...
int main(void)
{
 ...
 /* Função que cria uma tarefa para controle da Serial */
  ...
  xTaskCreate(vCounter1Task, "Contador1", 100, NULL, osPriorityNormal, NULL);
 /* Função que cria uma tarefa para o contador */
  xTaskCreate(vCounter2Task, "Contador2", 100, NULL, osPriorityNormal, NULL);
...
void vCounter1Task(void *pvParameters)
{
    ...
    {
        counter1++;
        //vTaskPrioritySet(NULL,osPriorityAboveNormal);
        vTaskDelay(500);
    }
}
